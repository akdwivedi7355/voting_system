...
def getOutput(status):
    ...
    con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'Chethan@123', database = 'evoting',charset='utf8')
    with con:
        cur = con.cursor()
        ...
        if face_component is not None:
            ...
            if(conf < 80):
                ...
                if validate_user == username:
                    status = "Successfull Voted and you cannot vote again"
        else:
            status = "Unable to detect face"
        if status == "Successfull Voted and you cannot vote again":
...
def AddPartyAction(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'Chethan@123', database = 'evoting',charset='utf8')
...
def AdminLogin(request):
    if request.method == 'POST':
        ...
        if username == 'admin' and password == 'admin':
            context= {'data':'Welcome '+username+' page'}
    ...
    if request.method == 'POST':
        ...
        if status == 'success':
            ...
            if hour >= 1 and hour < 24:
                ...
                context= {'data':'Login & Voting will be allowed 24 hour'}
